...
def process_one(data_id, link, save_dir):
    ...
    try:
        ...
        for item in ofs_data['features']:
            if item['message']['featureType'] in ['revolve']:
                revolve_flag = True
            #print(item['message']['featureType'])
            if item['message']['featureType'] not in ['newSketch', 'extrude', 'revolve']:
                if revolve_flag:
                    with open('revolve/' + data_id, 'w') as f:
                        f.write("revolve")
                return 0
...
def process_one_self(link):
    ...
    # parse detailed cad operations
    try:
        parser = FeatureListParser(c, did, wid, eid)
        result = parser.parse()
    except Exception as e:
        print("[{}], feature parsing fails:".format(data_id), e)
        return 0
...
#process_one_self("https://cad.onshape.com/documents/3d08e8ba0a8bfce8e4ef7405/w/db72ad767f0deb5b7894b0f0/e/d569b775f94d047157dcfb94")
